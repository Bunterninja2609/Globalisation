...
function entity:update(dt, _)
    ...
    if self.hitCooldownTimer <= 0 then
        ...
        if love.math.random(1, 20) == 20 then
            self.movementModifier = self.movementModifier * (-1)
        end
        self.movementTimer = self.movementTimer + 4*dt * self.movementModifier
        local baseDir = getDirection(player.x, player.y, self.x, self.y)
        local r = 50
